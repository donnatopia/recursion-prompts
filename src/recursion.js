...
var modulo = function(x, y) {
  ...
  if (difference < 0) {
    ...
    remainder = modulo(difference, y);
  }

  return negativeResult ? -remainder : remainder;
};
...
var minimizeZeroes = function(array) {
  if (array.length > 1) {
    var minimized = [array[0]];

    if (array[0] !== 0) {
      minimized.push(array[1]);
    } else if (array[1] !== 0) {
      minimized.push(array[1]);
    }

    if (array.length > 2) {
      var last = [minimized.pop()];
      var rest = last.concat(array.slice(2));
      minimized = minimized.concat(minimizeZeroes(rest));
    }
...
var alternateSign = function(array) {
  if (array.length > 1) {
    var alternateSignArr = [Math.abs(array[0]), -Math.abs(array[1])];

    if (array.length > 2) {
      alternateSignArr = alternateSignArr.concat(alternateSign(array.slice(2)));
    }

    return alternateSignArr;
  } else {
